...
import { createStorageScope } from "../../apps/workers/storage-scope.mjs";
import { createStorage } from "../../apps/workers/storage.mjs";

let boot;
const publicWorker = createMarketplaceWorker({
  diagnostics: true,
  onReceipt(receipt, response) {
    ...
    response.headers.set("x-proof-boot", boot);
  },
});
