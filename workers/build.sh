...
if ! "$CARGO" +1.94.0 rustc --locked --manifest-path "$root/Cargo.toml" \
  --target wasm32-unknown-unknown -p lenso-marketplace-workers-host --release \
...
for line in open(sys.argv[1]):
    item=json.loads(line)
    if item.get('reason')=='compiler-artifact' and item['target']['name']=='lenso_marketplace_workers_host':
...
runtime_root="${LENSO_RUNTIME_ROOT:-$root/../../../../../lenso-runtime-rust/design-workers-compatibility}"
mkdir -p "$root/runtime"
cp "$runtime_root/experiments/workers-runtime/http.mjs" "$root/runtime/http.mjs"
cp "$runtime_root/experiments/workers-runtime/runner.mjs" "$root/runtime/runner.mjs"
cp "$runtime_root/experiments/workers-g1/clock.mjs" "$root/clock.mjs"
