...
use lenso_marketplace_web_plugin::cache_storage::{AcceptedEnvelope, CacheStorage, EventCache};
use lenso_plugin_catalog::{Snapshot, Trust, sign};
use std::{
    cell::{Cell, RefCell},
...
fn concurrent_winner_is_reverified_before_any_overwrite() {
    block_on(async {
        ...
        let verified = lenso_plugin_catalog::verify(&newer, &trust, None, 150).unwrap();
        store.race.replace(Some(AcceptedEnvelope {
            token: lenso_plugin_catalog::digest(&newer),
